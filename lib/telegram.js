...
export async function handler(req, method) {
  ...
  try {
    ...
    await sendMessage(messageObj, response);
  } catch (error) {
...
export async function sendMessage(messageObj, text) {
  ...
  try {
    ...
    const response = await fetch(
      `https://api.telegram.org/bot${TELEGRAM_TOKEN}/sendMessage`,
      {
        ...
        body: JSON.stringify({
          chat_id: chatId,
          text: `Message sent by ${messageObj.from.first_name} ${messageObj.from.last_name}: ${text}`,
          parse_mode: "Markdown",
        }),
      }
    );
