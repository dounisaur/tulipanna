...
// OrderNumberExtractorTool class definition
...
// CustomerNameExtractorTool class definition
...
// CustomerOrdersTool class definition
...
// OrderDetailsTool class definition
export class OrderDetailsTool {
  ...
  async execute(orderNumber) {
    try {
      ...
      // Format line items if they exist
      let lineItemsText = "No items found";
      if (orderDetails.line_items && orderDetails.line_items.length > 0) {
        lineItemsText = orderDetails.line_items
          .map((item) => item.name)
          .join(", ");
      }
...
// DateExtractorTool class definition
export class DateExtractorTool {
  constructor() {
    this.llm = llm;
  }
  ...
  //====================================================================================
  /**
   * Executes the date extraction tool
   * @param {string} message - The message to extract the date from
   * @returns {string} The extracted date in MM/DD/YYYY format or null if not found
   */
  //====================================================================================
  async execute(message) {
    const prompt = `Extract the date from the following message. The date can be in various formats.
    If there is no date, respond with "NONE".
    Convert the date to MM/DD/YYYY format (e.g., 3/21/2025).
    Message: ${message}
    
    Examples:
    - "show me sales for 3/21/2025" -> 3/21/2025
    - "what were the sales on march 21st 2025" -> 3/21/2025
    - "give me data for March 21, 2025" -> 3/21/2025
    - "show me sales for 21/3/2025" -> 3/21/2025
    - "what were the sales on 21st March 2025" -> 3/21/2025
    - "show me all orders" -> NONE
    - "help me with my order" -> NONE
    
    Respond with ONLY the date in MM/DD/YYYY format or "NONE".`;

    console.log("\nExtracting date...");
    const response = await this.llm.invoke(prompt);
    const date = response.content.trim();
    console.log("Extracted date:", date);

    return date === "NONE" ? null : date;
  }
}
