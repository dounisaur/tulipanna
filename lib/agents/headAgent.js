...
import { analyzePastOperations, processMessage } from "../ragUtils.js";
...
export class HeadAgent {
  ...
  async analyze(message, userId) {
    ...
    // First, determine if this is an operational or analytical question
    ...
    const questionType = await processMessage(userId, questionTypePrompt);
    const isAnalytical = questionType.trim() === "ANALYTICAL";

    if (isAnalytical) {
      console.log("📊 This is an analytical question about past operations");
      const analysis = await analyzePastOperations(userId, message);
      return {
        isAnalytical: true,
        response: analysis,
      };
    ...
    const response = await processMessage(userId, prompt);
    const agentType = response.trim();
