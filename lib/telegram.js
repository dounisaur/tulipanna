...
export async function handler(req, method) {
  ...
  try {
    ...
    const agentType = await headAgent.analyze(messageText);
    ...
    switch (agentType) {
      ...
      case "ORDER_DETAILS_AGENT":
        response = await orderDetailsAgent.handle(messageText);
        break;
      case "ORDER_STATUS_AGENT":
        response = await orderStatusAgent.handle(messageText);
        break;
      case "HELP_AGENT":
