...
export async function setTelegramWebhook(TELEGRAM_TOKEN, WEBHOOK_URL) {
  console.log("I am in the function setTelegramWebhook");
  try {
...
export async function sendMessageToTelegram(botToken, chatId, text) {
  console.log("I am in the function sendMessageToTelegram");
  if (!text || text.trim() === "") {
    console.error(`Error sending message to chat ID ${chatId}: Text is empty`);
    return;
  }
      ...
      switch (chatType) {
        case "daily":
          ...
          if (userMessage.includes("/daily")) {
            // Get the store data from yesterday
            const dailyData = await getOrders();

            // TEST BELOW

            const orderObject = dailyData[0];
        const orderDetails = `
        ...
        console.log("orderDetails", orderDetails);
            ...
            // Conditionally generate a daily summary using the LLM or an error message if no data is available
            /*const dailySummary =
              dailyData.length > 0
                ? await respondToUserWithDailySummary(chatId, dailyData)
                : "Hi, i don't seem to have any data on yesterday's store performances. If you think i'm mistaken, please let Andrew know.";
           */
            // Log the response that will be sent
            console.log("response to telegram is: " + dailyData);
            ...
            await clearSessionData(chatId, "daily", req);
            await sendMessageToTelegram(
              botToken,
              chatId,
              "Session history and Airtable data cleared for chatID: " +
                chatId
            );
          } else {
            // Handle any other messages using the LLM
